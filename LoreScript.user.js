...
(function() {
    ...
    const waitForElement = (selector, timeout=10000) => new Promise((resolve,reject)=>{
        ...
        if(timeout) setTimeout(()=>{obs.disconnect(); reject(`Timeout waiting for ${selector}`);},timeout);
    });
    const getTradePageCash=()=>{ const el=document.querySelector('.money-value'); return el?parseNumber(el.textContent):0; };
    ...
    async function addBankButton(){
        ...
        btn.className="torn-btn orange"; btn.id="customTradeBtn";
    ...
    function addGhostButtons(input,sync){
        ...
        const mkBtn=(label,action)=>{
            ...
            b.className="torn-btn orange"; b.style.cssText="top:3px;display:block";
    ...
    new MutationObserver(()=>{
        if(location.pathname==='/trade.php') initTradePage();
    }).observe(document.body,{childList:true,subtree:true});

    window.addEventListener("hashchange",initTradePage);
    initTradePage();
